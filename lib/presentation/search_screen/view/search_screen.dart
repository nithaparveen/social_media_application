              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                    ...
                    title: Text(
                        "${controller.searchModel?.data?[index].username}"),
                    subtitle:
                        Text("${controller.searchModel?.data?[index].name}"),
                    trailing: SizedBox(
                      width: 100,
                      child: ElevatedButton(
                        onPressed: () {
                          Provider.of<HomeController>(context, listen: false)
                              .followTapped(
                                  controller.searchModel?.data?[index].id,
                                  context);
                        },
                        style: ElevatedButton.styleFrom(
                            backgroundColor: controller.searchModel
                                        ?.data?[index].isFollowing ==
                                    true
                                ? ColorTheme.white
                                : ColorTheme.blue),
                        child:
                            controller.searchModel?.data?[index].isFollowing ==
                                    true
                                ? Text(
                                    "Unfollow",
